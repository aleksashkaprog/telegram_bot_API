...
from keyboards.reply.reply import keyboard_yesno, keyboard_city, keyboard_number, keyboard_noyes
...
def get_info(message):
    ...
    if message.text.lower() == 'нет':
        ...
        for i in find_hotels(city_id=find_destinationid(command_list[1]), hotels_count=int(command_list[4]),
                             checkin_date=command_list[2], checkout_date=command_list[3], sortOrder="PRICE",
                             count_photo=0):
...
def get_info_with_photo(message):
    ...
    for i in find_hotels(city_id=find_destinationid(command_list[1]), hotels_count=int(command_list[4]),
                         checkin_date=command_list[2], checkout_date=command_list[3], sortOrder="PRICE",
                         count_photo=int(command_list[5])):
        bot.send_message(message.from_user.id, i)
        history_string += i
    us_id = message.from_user.id
    us_name = message.from_user.username
